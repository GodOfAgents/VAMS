...
def validate_credential_incident_report(
    ...
    for index, identity in enumerate(identities):
        ...
        if identity.get("decommission_disposition") != (
            "permanently-decommissioned-no-replacement"
        ):
            errors.append(f"{label} must record permanent decommission without replacement")
        decommissioned_at = _utc(
            identity.get("decommissioned_at"), f"{label}.decommissioned_at", errors
        )
        if reviewed_at and decommissioned_at and decommissioned_at > reviewed_at:
            errors.append(f"{label} decommissioning postdates the review")
        ...
        _artifact(
            identity.get("decommission_evidence"),
            root,
            f"{label}.decommission_evidence",
            errors,
        )
    ...
    if not isinstance(provider_credentials, list) or len(provider_credentials) != 1:
        errors.append("credential incident report requires exactly one Infura credential record")
        provider_credentials = []
    provider_fields = {
        "provider",
        "fingerprint_sha256",
    ...
    for index, provider in enumerate(provider_credentials):
        ...
        fingerprint = provider.get("fingerprint_sha256")
        if not isinstance(fingerprint, str) or HASH_RE.fullmatch(fingerprint) is None:
            errors.append(f"{label}.fingerprint_sha256 is invalid")
        ...
        if provider.get("exact_revocation_time_unavailable") is not True:
            errors.append(f"{label} must explicitly record the unavailable exact time")
        ...
        if revoked_before and observed_at and revoked_before > observed_at:
            errors.append(f"{label}.revoked_before must not postdate dashboard observation")
        if reviewed_at and observed_at and observed_at > reviewed_at:
            errors.append(f"{label}.observed_at postdates the review")
        if provider.get("access_review_clear") is not True:
            errors.append(f"{label} access review is not clear")
        if provider.get("billing_review_clear") is not True:
            errors.append(f"{label} billing review is not clear")
        _artifact(provider.get("evidence"), root, f"{label}.evidence", errors)
