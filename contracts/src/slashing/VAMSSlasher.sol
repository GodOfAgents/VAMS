 ...
 */
contract VAMSSlasher is Initializable, AccessControlUpgradeable, ReentrancyGuard {
    ...
    event OperatorSlashed(
        address indexed operator, OffenseType indexed offense, uint256 slashAmount, uint256 jailDuration
    );
    ...
    function initialize(address _admin, address _insuranceFund, address _vamsToken) public initializer {
    ...
    function withdrawStake(address _operator) external onlyRole(DEFAULT_ADMIN_ROLE) nonReentrant {
        ...
        if (op.status == OperatorStatus.JAILED && block.timestamp < op.jailedUntil) {
            revert OperatorStillJailed(op.jailedUntil);
        }
     ...
     */
    function slashLiveness(address _operator, uint256 _missedBlocks)
        external
        onlyRole(SLASHER_ROLE)
        nonReentrant
        returns (SlashResult memory result)
    {
        OperatorRecord storage op = operators[_operator];
     ...
     */
    function slashEquivocation(address _operator, bytes calldata _proof)
        external
        onlyRole(SLASHER_ROLE)
        nonReentrant
        returns (SlashResult memory result)
     ...
     */
    function slashMalicious(address _operator, address _victim, bytes calldata _evidence)
        external
        onlyRole(SLASHER_ROLE)
        nonReentrant
        returns (SlashResult memory result)
     ...
     */
    function slashSLA(address _operator, OffenseType _offenseType, uint256 _customBps)
        external
        onlyRole(SLASHER_ROLE)
        nonReentrant
        returns (SlashResult memory result)
    ...
    function _calculateLivenessSlash(uint256 _stake, uint256 _missedBlocks, uint256 _priorOffenses)
        internal
        pure
        returns (SlashResult memory result)
    {
        ...
        for (uint256 i = 0; i < maxIters; i++) {
            multiplier =
                (multiplier * SlashingParameters.SLASH_REPEAT_MULTIPLIER_BPS) / SlashingParameters.BPS_DENOMINATOR;
        ...
        result.slashAmount = (_stake * penaltyBps * multiplier)
            / (SlashingParameters.BPS_DENOMINATOR * SlashingParameters.BPS_DENOMINATOR);
        result.burnAmount = result.slashAmount; // 100% burned for liveness
    }

    function _calculateEquivocationSlash(uint256 _stake, uint256 _priorEquivocations)
        internal
        pure
        returns (SlashResult memory result)
    {
    ...
    function _calculateMaliciousSlash(uint256 _stake, address _victim)
        internal
        pure
        returns (SlashResult memory result)
    {
        result.slashAmount = (_stake * SlashingParameters.SLASH_MALICIOUS_PENALTY_BPS)
            / SlashingParameters.BPS_DENOMINATOR;
        result.tombstone = true;

        if (_victim != address(0)) {
            result.victimCompensation = (result.slashAmount * SlashingParameters.SLASH_MALICIOUS_VICTIM_SHARE_BPS)
                / SlashingParameters.BPS_DENOMINATOR;
        }
    ...
    function _executeSlash(address _operator, OperatorRecord storage _op, SlashResult memory _result) internal {
        // Reduce stake
    ...
    function getOperatorStatus(address _operator)
        external
        view
        returns (
            uint256 stake,
    ...
    {
        OperatorRecord storage op = operators[_operator];
        return (op.stake, op.status, op.jailedUntil, op.livenessOffenses, op.equivocationOffenses, op.totalSlashed);
    }
     ...
     */
    function _verifyEquivocationProof(address _operator, bytes calldata _proof) internal view returns (bool) {
     ...
     */
    function _validateMaliciousEvidence(address _operator, address _victim, bytes calldata _evidence)
        internal
        view
        returns (bool)
    {
