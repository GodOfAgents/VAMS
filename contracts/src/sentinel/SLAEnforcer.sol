...
contract SLAEnforcer is Initializable, AccessControlUpgradeable, ReentrancyGuard, ISLAEnforcer {
    ...
    function initialize(address _admin, address _registry, address _slasher) public initializer {
    ...
    function submitSLAReport(SLAReport calldata report, bytes calldata signature) external nonReentrant {
        ...
        try registry.updateBenchmark(report.nodeId, report.metricsScore, report.timestamp) {}
