 ...
 */
contract VAMSAgentRegistry is Initializable, AccessControlUpgradeable, ReentrancyGuard, PausableUpgradeable {
    ...
    event AgentRegistered(bytes32 indexed agentId, address indexed owner, uint256 stake, uint256 finalizedAt);
    ...
    event MerkleRootSubmitted(bytes32 indexed agentId, bytes32 merkleRoot, uint256 blockNumber);

    event HeartbeatReceived(bytes32 indexed agentId, uint256 timestamp);

    event ChallengeSubmitted(
        bytes32 indexed challengeId, bytes32 indexed agentId, address indexed challenger, uint256 stake
    );

    event ChallengeResolved(bytes32 indexed challengeId, ChallengeStatus status, uint256 slashAmount);
    ...
    function initialize(address _admin, address _vamsToken, address _slasher) public initializer {
     ...
     */
    function registerAgent(bytes32 _publicKeyHash, uint256 _stake, string calldata _metadata, address _authorizedWallet)
        external
        nonReentrant
        whenNotPaused
     ...
     */
    function submitMerkleRoot(bytes32 _agentId, bytes32 _merkleRoot) external nonReentrant {
        Agent storage agent = agents[_agentId];
        ...
        merkleHistory[_agentId].push(
            MerkleSubmission({merkleRoot: _merkleRoot, blockNumber: block.number, timestamp: block.timestamp})
        );
     ...
     */
    function updateTEEAttestation(bytes32 _agentId, bytes32 _attestation) external nonReentrant {
        Agent storage agent = agents[_agentId];

        if (agent.owner != msg.sender && agent.authorizedWallet != msg.sender && !hasRole(VERIFIER_ROLE, msg.sender)) {
     ...
     */
    function submitChallenge(bytes32 _agentId, bytes32 _fraudProofHash, uint256 _stake, string calldata _evidence)
        external
        nonReentrant
        whenNotPaused
