...
contract VAMSRouter is VAMSUpgradeableBase {
     ...
     */
    function routeTransaction(bytes32 _txHash, TransactionMetadata calldata _metadata)
        external
        onlyRole(ROUTER_OPERATOR_ROLE)
        returns (RoutingDecision memory decision)
    {
        ...
        decision = RoutingDecision({
            targetChain: target, isManualOverride: false, decidedBy: msg.sender, timestamp: block.timestamp
        });
     ...
     */
    function setManualRoute(bytes32 _txHash, ChainId _targetChain, uint256 _duration, string calldata _reason)
        external
        onlyRole(MANUAL_ROUTER_ROLE)
    {
        _checkManualRoutingAllowed();

        if (_duration > MAX_MANUAL_ROUTE_DURATION) revert InvalidDuration();

        manualRoutes[_txHash] =
            ManualRoute({targetChain: _targetChain, expiresAt: block.timestamp + _duration, reason: _reason});
     ...
     */
    function activateGlobalOverride(ChainId _targetChain, uint256 _duration, string calldata _reason)
        external
        onlyRole(MANUAL_ROUTER_ROLE)
    {
        _checkManualRoutingAllowed();
