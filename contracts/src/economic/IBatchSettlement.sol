...
interface IBatchSettlement {
    ...
    event PaymentClaimed(
        uint256 indexed batchId, bytes32 indexed paymentHash, address indexed provider, uint256 amount
    );

    event PaymentDisputed(uint256 indexed batchId, bytes32 indexed paymentHash, address indexed disputer);

    event PaymentRefunded(uint256 indexed batchId, bytes32 indexed paymentHash, address indexed agent, uint256 amount);
     ...
     */
    function claimPayment(uint256 batchId, Payment calldata payment, bytes32[] calldata merkleProof) external;
     ...
     */
    function resolvePaymentDispute(bytes32 paymentHash, bool refundAgent) external;
}
