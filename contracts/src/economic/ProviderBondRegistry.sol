    ...
    IProviderBondRegistry
{
     ...
     */
    function initialize(address admin, address _vamsToken, address _insuranceFund) external initializer {
    ...
    function getBond(address provider) external view override returns (ProviderBond memory) {
    ...
    /// @inheritdoc IProviderBondRegistry
    function getActiveRequest(bytes32 escrowId) external view override returns (ActiveRequest memory) {
    ...
    /// @inheritdoc IProviderBondRegistry
    function getAvailableBond(address provider) external view override returns (uint256) {
        ProviderBond storage bond = _bonds[provider];
    ...
    /// @inheritdoc IProviderBondRegistry
    function registerProvider(uint256 bondAmount, uint256 maxRequestValue)
        external
        override
        nonReentrant
        whenNotPaused
    ...
    /// @inheritdoc IProviderBondRegistry
    function increaseBond(uint256 additionalAmount) external override nonReentrant whenNotPaused {
    ...
    /// @inheritdoc IProviderBondRegistry
    function updateMaxRequestValue(uint256 newMaxRequestValue) external override whenNotPaused {
        ProviderBond storage bond = _bonds[msg.sender];
        ...
        if (bond.bondedAmount < newMaxRequestValue * BOND_COVERAGE_RATIO) {
            revert CoverageRatioNotMet(bond.bondedAmount, newMaxRequestValue, BOND_COVERAGE_RATIO);
        }
    ...
    /// @inheritdoc IProviderBondRegistry
    function requestWithdrawal(uint256 amount) external override whenNotPaused {
        ProviderBond storage bond = _bonds[msg.sender];
        ...
        if (remainingBond > 0 && remainingBond < bond.maxRequestValue * BOND_COVERAGE_RATIO) {
            revert CoverageRatioNotMet(remainingBond, bond.maxRequestValue, BOND_COVERAGE_RATIO);
        }
    ...
    /// @inheritdoc IProviderBondRegistry
    function acceptRequest(address provider, bytes32 escrowId, uint256 value)
        external
        override
        ...
        whenNotPaused
    {
        ...
        _activeRequests[escrowId] = ActiveRequest({value: value, startedAt: block.timestamp, provider: provider});
    ...
    /// @inheritdoc IProviderBondRegistry
    function completeRequest(address provider, bytes32 escrowId) external override onlyRole(ESCROW_ROLE) {
    ...
    /// @inheritdoc IProviderBondRegistry
    function slashProvider(address provider, uint256 amount, bytes32 escrowId, string calldata reason)
        external
        override
    ...
    /// @inheritdoc IProviderBondRegistry
    function compensateProvider(address provider, uint256 amount, bytes32 escrowId)
        external
        override
        onlyRole(ESCROW_ROLE)
        nonReentrant
     ...
     */
    function forceDeactivate(address provider, string calldata reason) external onlyRole(ADMIN_ROLE) {
        ProviderBond storage bond = _bonds[provider];
