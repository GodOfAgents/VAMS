 ...
 */
contract X402NonceRegistry is Initializable, AccessControlUpgradeable, PausableUpgradeable, IX402NonceRegistry {
     ...
     */
    function initialize(address admin, address _escrowManager) external initializer {
    ...
    function isNonceValid(address agent, uint256 nonce) external view override returns (bool) {
        return !_nonces[agent][nonce].used;
    }

    /// @inheritdoc IX402NonceRegistry
    function isReceiptClaimed(bytes32 receiptHash) external view override returns (bool) {
    ...
    /// @inheritdoc IX402NonceRegistry
    function getNextNonce(address agent) external view override returns (uint256) {
    ...
    /// @inheritdoc IX402NonceRegistry
    function getHighestUsedNonce(address agent) external view override returns (uint256) {
        return _highestNonce[agent];
    }

    /// @inheritdoc IX402NonceRegistry
    function getNonceInfo(address agent, uint256 nonce) external view override returns (NonceInfo memory) {
        return _nonces[agent][nonce];
    }

    /// @inheritdoc IX402NonceRegistry
    function isAuthorizedSettler(address settler) external view override returns (bool) {
    ...
    /// @inheritdoc IX402NonceRegistry
    function consumeNonce(address agent, uint256 nonce, bytes32 receiptHash, bytes32 escrowId)
        external
        override
    ...
    function batchConsumeNonces(
        ...
        require(
            length == nonces.length && length == receiptHashes.length && length == escrowIds.length,
    ...
    /// @inheritdoc IX402NonceRegistry
    function authorizeSettler(address settler) external override onlyRole(ADMIN_ROLE) {
    ...
    /// @inheritdoc IX402NonceRegistry
    function deauthorizeSettler(address settler) external override onlyRole(ADMIN_ROLE) {
     ...
     */
    function _consumeNonce(address agent, uint256 nonce, bytes32 receiptHash, bytes32 escrowId) internal {
        ...
        // Mark nonce as used
        _nonces[agent][nonce] =
            NonceInfo({used: true, receiptHash: receiptHash, consumedAt: block.timestamp, escrowId: escrowId});
     ...
     * @return Receipt hash
     */
    function _computeReceiptHash(address agent, uint256 nonce, uint256 amount) internal pure returns (bytes32) {
