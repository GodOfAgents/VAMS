    ...
    IVAMSPaymentHandler
{
     ...
     */
    function initialize(address _admin, address _vamsToken, address _treasury) public initializer {
    ...
    function openChannel(address counterparty, uint256 deposit)
        external
        override
        nonReentrant
        whenNotPaused
        returns (bytes32 channelId)
    {
        ...
        channelId = keccak256(abi.encodePacked(msg.sender, counterparty, block.timestamp, totalChannels));
    ...
    /// @inheritdoc IVAMSPaymentHandler
    function initiateClose(bytes32 channelId, ChannelState calldata state, bytes calldata signature)
        external
        override
        nonReentrant
    {
    ...
    /// @inheritdoc IVAMSPaymentHandler
    function dispute(bytes32 channelId, ChannelState calldata state, bytes calldata signature)
        external
        override
        nonReentrant
    {
     ...
     */
    function _executeSettlement(bytes32 channelId, uint256 balanceA, uint256 balanceB) internal {
        Channel storage channel = _channels[channelId];
    ...
    function _hashState(ChannelState calldata state) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(state.channelId, state.nonce, state.balanceA, state.balanceB));
    }

    /**
     * @notice Verify signature matches expected signer
     */
    function _verifySignature(bytes32 hash, bytes calldata signature, address expectedSigner) internal pure {
    ...
    function isDisputable(bytes32 channelId) external view override returns (bool) {
        Channel storage channel = _channels[channelId];
        return channel.status == ChannelStatus.CLOSING && block.timestamp < channel.closingAt;
    }
