    ...
    ITwoPhaseCommitManager
{
     ...
     */
    function initialize(address admin, address _vamsToken, address _escrowManager) external initializer {
    ...
    /// @inheritdoc ITwoPhaseCommitManager
    function getService(bytes32 workflowId, bytes32 escrowId) external view override returns (ServiceEntry memory) {
        return _services[workflowId][escrowId];
    }

    /// @inheritdoc ITwoPhaseCommitManager
    function getWorkflowEscrows(bytes32 workflowId) external view override returns (bytes32[] memory) {
        return _workflowEscrows[workflowId];
    }

    /// @inheritdoc ITwoPhaseCommitManager
    function isReadyToCommit(bytes32 workflowId) external view override returns (bool) {
        Workflow storage wf = _workflows[workflowId];
        return wf.state == WorkflowState.PREPARED
            || (wf.state == WorkflowState.PENDING && wf.preparedCount == wf.serviceCount);
    }

    /// @inheritdoc ITwoPhaseCommitManager
    function isExpired(bytes32 workflowId) external view override returns (bool) {
        Workflow storage wf = _workflows[workflowId];

        if (wf.state == WorkflowState.EXPIRED) return true;

        // Check prepare deadline
        if (wf.state == WorkflowState.PENDING && block.timestamp > wf.prepareDeadline) {
            return true;
        }

        // Check commit deadline
        if (wf.state == WorkflowState.PREPARED && block.timestamp > wf.commitDeadline) {
            return true;
        }
    ...
    function initiateWorkflow(
        ...
        if (_workflows[workflowId].createdAt != 0) {
            revert InvalidWorkflowState(workflowId, _workflows[workflowId].state, WorkflowState.PENDING);
        }

        uint256 actualPrepareWindow = prepareWindow > 0 ? prepareWindow : DEFAULT_PREPARE_WINDOW;
        ...
        emit WorkflowInitiated(
            workflowId, msg.sender, services.length, totalValue, _workflows[workflowId].prepareDeadline
        );
    ...
    /// @inheritdoc ITwoPhaseCommitManager
    function commitWorkflow(bytes32 workflowId, bytes32 secret) external override whenNotPaused {
        Workflow storage wf = _workflows[workflowId];
        ...
        if (wf.state == WorkflowState.PENDING && wf.preparedCount == wf.serviceCount) {
        ...
        if (keccak256(abi.encodePacked(secret)) != wf.commitSecretHash) {
            revert InvalidCommitSecret(keccak256(abi.encodePacked(secret)), wf.commitSecretHash);
        }
    ...
    /// @inheritdoc ITwoPhaseCommitManager
    function abortWorkflow(bytes32 workflowId, string calldata reason) external override {
        Workflow storage wf = _workflows[workflowId];
        ...
        if (wf.state != WorkflowState.PENDING && wf.state != WorkflowState.PREPARED) {
    ...
    /// @inheritdoc ITwoPhaseCommitManager
    function markServicePrepared(bytes32 workflowId, bytes32 escrowId) external override {
        Workflow storage wf = _workflows[workflowId];
    ...
    function processExpiredWorkflow(bytes32 workflowId) external override {
        ...
        if (wf.state == WorkflowState.PENDING && block.timestamp > wf.prepareDeadline) {
            expired = true;
        } else if (wf.state == WorkflowState.PREPARED && block.timestamp > wf.commitDeadline) {
            expired = true;
        }
    ...
    /// @inheritdoc ITwoPhaseCommitManager
    function claimRefund(bytes32 workflowId, bytes32 escrowId) external override nonReentrant {
        Workflow storage wf = _workflows[workflowId];
        ServiceEntry storage svc = _services[workflowId][escrowId];
        ...
        if (wf.state != WorkflowState.ABORTED && wf.state != WorkflowState.EXPIRED) {
     ...
     */
    function getRevealedSecret(bytes32 workflowId) external view returns (bytes32) {
     ...
     */
    function getAgentWorkflows(address agent) external view returns (bytes32[] memory) {
     ...
     */
    function setEscrowManager(address _escrowManager) external onlyRole(ADMIN_ROLE) {
        escrowManager = IX402EscrowManager(_escrowManager);
    }
