...
interface IX402EscrowManager {
    ...
    event EscrowClaimed(bytes32 indexed escrowId, address indexed provider, uint256 amount, ProofType proofType);

    event EscrowRefunded(bytes32 indexed escrowId, address indexed agent, uint256 amount);

    event EscrowDisputed(bytes32 indexed escrowId, address indexed disputer, bytes evidence);

    event DisputeResolved(
        bytes32 indexed escrowId, address indexed winner, uint256 agentRefund, uint256 providerPayout
    );

    event EscrowExtended(bytes32 indexed escrowId, uint256 oldExpiry, uint256 newExpiry);

    event ServiceProofApproved(bytes32 indexed escrowId, address indexed verifier);
     ...
     */
    function claimEscrow(bytes32 escrowId, ServiceProof calldata serviceProof, bytes32 preimage) external;
     ...
     */
    function resolveDispute(bytes32 escrowId, address winner, uint256 agentRefund, uint256 providerPayout) external;
}
