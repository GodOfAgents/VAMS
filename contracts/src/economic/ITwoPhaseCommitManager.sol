...
interface ITwoPhaseCommitManager {
    ...
    event ServicePrepared(bytes32 indexed workflowId, bytes32 indexed escrowId, address indexed provider);
    ...
    event ServiceClaimed(
        bytes32 indexed workflowId, bytes32 indexed escrowId, address indexed provider, uint256 amount
    );

    event ServiceRefunded(bytes32 indexed workflowId, bytes32 indexed escrowId, address indexed agent, uint256 amount);
     ...
     */
    function getService(bytes32 workflowId, bytes32 escrowId) external view returns (ServiceEntry memory);
     ...
     */
    function getWorkflowEscrows(bytes32 workflowId) external view returns (bytes32[] memory);
