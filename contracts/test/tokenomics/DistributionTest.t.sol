...
contract DistributionTest is Test {
    ...
    function test_SovereignDistribution() public {
        // Community: 400M (40%)
        bytes32 commId = vesting.createVestingSchedule(deployer, 400_000_000 * 1e18, IVAMSVesting.ScheduleType.COMMUNITY, true);
        ...
        // Founder: 120M (12%)
        bytes32 founderId = vesting.createVestingSchedule(deployer, 120_000_000 * 1e18, IVAMSVesting.ScheduleType.FOUNDER, true);
        ...
        // Future Team: 130M (13%)
        bytes32 teamId = vesting.createVestingSchedule(deployer, 130_000_000 * 1e18, IVAMSVesting.ScheduleType.TEAM, true);
        assertEq(vesting.getSchedule(teamId).totalAmount, 130_000_000 * 1e18, "Team should be 130M");

        // Early Investors: 50M (5%)
        bytes32 earlyInvId = vesting.createVestingSchedule(deployer, 50_000_000 * 1e18, IVAMSVesting.ScheduleType.EARLY_INVESTOR, true);
        assertEq(vesting.getSchedule(earlyInvId).totalAmount, 50_000_000 * 1e18, "Early Investor should be 50M");

        // Regular Investors: 80M (8%)
        bytes32 regInvId = vesting.createVestingSchedule(deployer, 80_000_000 * 1e18, IVAMSVesting.ScheduleType.REG_INVESTOR, true);
        ...
        bytes32 treasId = vesting.createVestingSchedule(deployer, 120_000_000 * 1e18, IVAMSVesting.ScheduleType.FOUNDATION, true);
