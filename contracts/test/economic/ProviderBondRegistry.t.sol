...
contract ProviderBondRegistryTest is BaseTest {
    ...
    function test_RegisterProvider_RevertsOnMinBond() public {
        ...
        vm.prank(provider1);
        vm.expectRevert(abi.encodeWithSelector(IProviderBondRegistry.BondTooLow.selector, bondAmount, MIN_BOND));
        registry.registerProvider(bondAmount, maxRequest);
    }
    ...
    function test_RegisterProvider_RevertsOnCoverageRatio() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IProviderBondRegistry.CoverageRatioNotMet.selector, bondAmount, maxRequest, COVERAGE_RATIO
            )
        );
        registry.registerProvider(bondAmount, maxRequest);
    }
    ...
    function test_RegisterProvider_RevertsOnDuplicate() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(IProviderBondRegistry.ProviderAlreadyRegistered.selector, provider1));
        registry.registerProvider(bondAmount, maxRequest);
        vm.stopPrank();
    }
    ...
    function test_UpdateMaxRequest_RevertsIfExceedsCoverage() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IProviderBondRegistry.CoverageRatioNotMet.selector, bondAmount, invalidMaxRequest, COVERAGE_RATIO
            )
        );
    ...
    function test_ExecuteWithdrawal_RevertsBeforeDelay() public {
        ...
        vm.prank(provider1);
        vm.expectRevert(
            abi.encodeWithSelector(IProviderBondRegistry.WithdrawalLocked.selector, block.timestamp + 1 hours)
        );
        registry.executeWithdrawal();
    }
    ...
    function test_AcceptRequest_RevertsOnExceedMax() public {
        ...
        vm.prank(user1);
        vm.expectRevert(
            abi.encodeWithSelector(IProviderBondRegistry.RequestValueTooHigh.selector, maxRequest + 1, maxRequest)
        );
    ...
    function test_SlashProvider_EmitsEvent() public {
        ...
        emit IProviderBondRegistry.ProviderSlashed(provider1, slashAmount, escrowId, "Service failure");
