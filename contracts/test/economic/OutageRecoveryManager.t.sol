...
contract OutageRecoveryManagerTest is BaseTest {
    ...
    event EmergencyWithdrawalRequested(uint256 indexed requestId, address indexed requester, address token, uint256 amount);
    ...
    function setUp() public override {
        ...
        bytes memory initData = abi.encodeWithSelector(
            OutageRecoveryManager.initialize.selector,
            admin,
            guardians,
            supportedTokens
        );
    ...
    function test_RequestEmergencyWithdraw_Revert_ExceedsLimit() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(
            IOutageRecoveryManager.InsufficientBalance.selector,
            address(token),
            excessiveAmount,
            maxWithdrawal
        ));
    ...
    function test_ProcessWithdrawal_Revert_NotRequester() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(
            IOutageRecoveryManager.NotRequester.selector,
            user2,
            user1
        ));
        vm.prank(user2);
