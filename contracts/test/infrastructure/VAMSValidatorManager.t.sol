...
contract MockStaking is IVAMSStaking {
    ...
    function compoundRewards() external override returns (uint256) {
        return 0;
    }
    ...
    function getTier(address) external pure override returns (StakingTier) {
        return StakingTier.None;
    }

    function isCLROperatorEligible(address) external pure override returns (bool) {
        return false;
    }
    ...
    function getAPY(StakingTier, LockPeriod) external pure override returns (uint256) {
        return 0;
    }

    function timeToUnlock(address) external pure override returns (uint256) {
        return 0;
    }

    function rewardPerSecond() external pure override returns (uint256) {
        return 0;
    }
    ...
    function annualEmissionBudget() external pure override returns (uint256) {
        return 0;
    }

    function annualMintedTokens() external pure override returns (uint256) {
        return 0;
    }

    function currentMintYearStart() external pure override returns (uint256) {
        return 0;
    }
...
contract MockFeeCollector is IVAMSFeeCollector {
    ...
    function getAccumulatedFees(address) external pure override returns (uint256) {
        return 0;
    }
    ...
    function getAutoDistributeThreshold() external pure override returns (uint256) {
        return 0;
    }
...
contract VAMSValidatorManagerTest is BaseTest {
    ...
    function test_UpdateLoyaltyTiers_SetsNewTiers() public {
        ...
        tiers[0] = IVAMSValidatorManager.LoyaltyTier({minStake: 200_000 ether, minLockDays: 180, discountBps: 150});
        tiers[1] = IVAMSValidatorManager.LoyaltyTier({minStake: 500_000 ether, minLockDays: 365, discountBps: 200});
    ...
    function testFuzz_CalculateMarkup_AlwaysWithinBounds(uint256 stakeAmount, uint256 stakeDays) public {
        stakeAmount = bound(stakeAmount, 0, 10_000_000 ether);
        stakeDays = bound(stakeDays, 0, 365);

        if (stakeAmount > 0 && stakeDays > 0) {
            mockStaking.setStakeInfo(enterprise1, stakeAmount, block.timestamp - (stakeDays * 1 days));
        }
    ...
    function testFuzz_UpdateBounds_RejectsInvalidRanges(uint256 minBps, uint256 maxBps) public {
