...
contract AgentLifecycleIntegration is BaseTest {
    ...
    function setUp() public override {
        ...
        vm.prank(admin);
        staking = new VAMSStaking(address(token), address(token), EMISSION_RATE, admin);
    ...
    function _registerAgent(address agentOwner, bytes32 publicKeyHash, string memory metadata, uint256 stake)
        internal
    ...
    function test_FullAgentLifecycle() public {
        ...
        bytes32 agentId = _registerAgent(agent1, publicKeyHash, "ipfs://agent-metadata", MIN_REGISTRATION_STAKE);
    ...
    function test_AgentChallenge_FraudProven() public {
        // 1. Register agent
        bytes32 agentId =
            _registerAgent(agent1, keccak256("SuspiciousAgent-PubKey"), "ipfs://suspicious", MIN_REGISTRATION_STAKE);
        ...
        vm.prank(user1);
        bytes32 challengeId =
            agentRegistry.submitChallenge(agentId, fraudProofHash, MIN_CHALLENGER_STAKE, "ipfs://evidence");
        ...
        assertEq(token.balanceOf(user1), challengerBefore + MIN_CHALLENGER_STAKE + reward);
    ...
    function test_AgentChallenge_Rejected() public {
        // 1. Register agent
        bytes32 agentId =
            _registerAgent(agent1, keccak256("HonestAgent-PubKey"), "ipfs://honest", MIN_REGISTRATION_STAKE);
        ...
        bytes32 challengeId = agentRegistry.submitChallenge(
            agentId, keccak256("false fraud proof"), MIN_CHALLENGER_STAKE, "ipfs://false-evidence"
        );
    ...
    function test_MultipleAgents_SameOwner() public {
        ...
        assertEq(uint256(status1), uint256(VAMSAgentRegistry.AgentStatus.ACTIVE));
        assertEq(uint256(status2), uint256(VAMSAgentRegistry.AgentStatus.ACTIVE));
        assertEq(uint256(status3), uint256(VAMSAgentRegistry.AgentStatus.ACTIVE));
    }
    ...
    function test_StakedUser_CanRegisterAgent() public {
        ...
        bytes32 agentId =
            _registerAgent(user1, keccak256("StakedUserAgent-PubKey"), "ipfs://staked-user", MIN_REGISTRATION_STAKE);
    ...
    function test_TEEAttestation_Update() public {
        // Register agent
        bytes32 agentId =
            _registerAgent(agent1, keccak256("TEEAgent-PubKey"), "ipfs://tee-agent", MIN_REGISTRATION_STAKE);
    ...
    function test_MerkleHistory_Tracked() public {
        ...
        bytes32 agentId =
            _registerAgent(agent1, keccak256("MerkleAgent-PubKey"), "ipfs://merkle", MIN_REGISTRATION_STAKE);
        _advanceTime(CHALLENGE_WINDOW + 1);
        vm.prank(agent1);
        agentRegistry.finalizeAgent(agentId);
    ...
    function test_CannotFinalize_BeforeWindow() public {
        bytes32 agentId = _registerAgent(agent1, keccak256("Test-PubKey"), "ipfs://test", MIN_REGISTRATION_STAKE);
    ...
    function test_ChallengeMustMeetMinStake() public {
        ...
        vm.prank(user1);
        vm.expectRevert();
        agentRegistry.submitChallenge(agentId, keccak256("fraud"), MIN_CHALLENGER_STAKE - 1, "ipfs://evidence");
    }
    ...
    function testFuzz_AgentStakeAmount(uint256 stakeAmount) public {
        ...
        bytes32 agentId =
            _registerAgent(agent1, keccak256(abi.encodePacked("Fuzz-PubKey", stakeAmount)), "ipfs://fuzz", stakeAmount);
