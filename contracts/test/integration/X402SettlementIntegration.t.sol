...
contract X402SettlementIntegration is BaseTest {
    ...
    function test_FullSettlementFlow_Success() public {
        ...
        vm.prank(agent1);
        bytes32 escrowId = escrow.lockEscrow(
            provider1,
            paymentAmount,
            nonce,
            ESCROW_VALIDITY,
        ...
        assertEq(uint(lockedEscrow.status), uint(IX402EscrowManager.EscrowStatus.LOCKED));
        ...
        assertEq(uint(claimedEscrow.status), uint(IX402EscrowManager.EscrowStatus.CLAIMED));
    ...
    function test_FullSettlementFlow_Refund() public {
        ...
        assertEq(uint(refundedEscrow.status), uint(IX402EscrowManager.EscrowStatus.REFUNDED));
    ...
    function test_FullSettlementFlow_Dispute() public {
        ...
        assertEq(uint(disputed.status), uint(IX402EscrowManager.EscrowStatus.DISPUTED));
    ...
    function test_HTLC_WithPreimage() public {
        ...
        assertEq(uint(claimed.status), uint(IX402EscrowManager.EscrowStatus.CLAIMED));
    }
