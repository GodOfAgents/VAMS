...
contract VAMSStakingTest is BaseTest {
    ...
    function test_Stake_SetsLockPeriod() public {
        ...
        IVAMSStaking.StakeInfo memory info = staking.getStakeInfo(user1);
        assertEq(uint256(info.lockPeriod), uint256(IVAMSStaking.LockPeriod.ThirtyDays));
    ...
    function test_Stake_EmitsEvent() public {
        ...
        emit IVAMSStaking.Staked(user1, amount, IVAMSStaking.LockPeriod.SevenDays, block.timestamp + 7 days);
    ...
    function test_Stake_RevertsOnBelowMinimum() public {
        ...
        vm.prank(user1);
        vm.expectRevert(abi.encodeWithSelector(IVAMSStaking.BelowMinimumStake.selector, amount, MIN_STAKE));
        staking.stake(amount, IVAMSStaking.LockPeriod.SevenDays);
    }

    // ============ Tier Tests ============

    function test_Tier_Silver_AtMinStake() public {
        uint256 amount = MIN_STAKE; // MIN_STAKE == SILVER_THRESHOLD
        ...
        assertEq(uint256(tier), uint256(IVAMSStaking.StakingTier.Silver));
    }
    ...
    function test_Tier_Gold_AtThreshold() public {
        ...
        IVAMSStaking.StakingTier tier = staking.getTier(user1);
        assertEq(uint256(tier), uint256(IVAMSStaking.StakingTier.Gold));
    }
    ...
    function test_Tier_Platinum_AtThreshold() public {
        ...
        IVAMSStaking.StakingTier tier = staking.getTier(user1);
        assertEq(uint256(tier), uint256(IVAMSStaking.StakingTier.Platinum));
    }

    function test_Tier_EmitsEventOnChange() public {
        // Start at Silver (MIN_STAKE = SILVER_THRESHOLD)
        _fundAndApprove(user1, address(staking), GOLD_THRESHOLD);

        vm.prank(user1);
        staking.stake(MIN_STAKE, IVAMSStaking.LockPeriod.SevenDays);

        // Add stake to reach Gold
        vm.expectEmit(true, false, false, true);
        emit IVAMSStaking.TierChanged(user1, IVAMSStaking.StakingTier.Silver, IVAMSStaking.StakingTier.Gold);
    ...
    function test_Unstake_RevertsBeforeLockExpiry() public {
        ...
        vm.prank(user1);
        vm.expectRevert(abi.encodeWithSelector(IVAMSStaking.StakeLocked.selector, info.lockedUntil));
    ...
    function test_Unstake_RevertsIfMoreThanStaked() public {
        ...
        vm.prank(user1);
        vm.expectRevert(abi.encodeWithSelector(IVAMSStaking.InsufficientStake.selector, amount + 1, amount));
    ...
    function test_Withdraw_RevertsBeforeUnbonding() public {
        ...
        vm.prank(user1);
        vm.expectRevert(abi.encodeWithSelector(IVAMSStaking.UnbondingNotComplete.selector, request.unbondingEnd));
        staking.withdraw();
    }
    ...
    function test_UpdateEmissionRate_Works() public {
        uint256 newRate = 7e14; // Below MAX_EMISSION_RATE
