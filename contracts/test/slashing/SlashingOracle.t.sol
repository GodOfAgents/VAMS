...
contract SlashingOracleTest is BaseTest {
    ...
    event ProposalFinalized(
        uint256 indexed proposalId, ISlashingOracle.ProposalStatus status, uint256 forStake, uint256 againstStake
    );
    ...
    function setUp() public override {
        ...
        bytes memory initData =
            abi.encodeWithSelector(SlashingOracle.initialize.selector, admin, address(vamsSlasher), address(staking));
    ...
    function _deploySlasher() internal returns (VAMSSlasher) {
        ...
        bytes memory initData =
            abi.encodeWithSelector(VAMSSlasher.initialize.selector, admin, insuranceFund, address(token));
